...
export default function Practice({ mode }: PracticeProps) {
  ...
  let presentation: 'static' | 'sequential' = 'static';
  ...
  const [sequenceReady, setSequenceReady] = useState(presentation === 'static');

  useEffect(() => {
    setAnswerStr('');
    setSequenceReady(presentation === 'static');
  }, [session.currentIndex, presentation]);
  ...
  const doSubmit = useCallback(() => {
    if (presentation === 'sequential' && !sequenceReady) return;
    const value = answerStr === '' || answerStr === '-' ? null : parseInt(answerStr, 10);
    const correct = value !== null && session.currentQuestion && value === session.currentQuestion.answer;
    setFeedback(correct ? 'correct' : 'wrong');
    setTimeout(() => setFeedback(null), 400);
    session.submitAnswer(value);
  }, [answerStr, session, presentation, sequenceReady]);
      ...
      {session.isPaused ? (
        ...
        <>
          ...
          >
            ...
            />
            {presentation === 'sequential' && !sequenceReady && (
          ...
          <div className="mt-6">
            ...
            <NumericKeypad
              ...
              onSubmit={doSubmit}
              disabled={presentation === 'sequential' && !sequenceReady}
            />
          </div>
        </>
      )}
